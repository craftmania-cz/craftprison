...
public class PCoinsCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (args.length == 0) {
                sender.sendMessage("§eAktualne mas §7" + PrisCoins.getCoins(p) + " §ePrisCoinu!");
                return true;
            }
            if (!sender.hasPermission("craftprison.priscoins.admin")) {
            ...
            switch (arg) {
                case "give":
                    if (args.length > 3 || args.length < 3) {
                        sender.sendMessage("§cSpatne argumenty!");
                        break;
                    }
                    try {
                        int value = Integer.valueOf(args[2]);
                        Player target = Bukkit.getPlayer(args[1]);
                        if (target == null) {
                            sender.sendMessage("§cHrac musi byt online");
                            break;
                        }
                        PrisCoins.giveCoins(target, value);
                        sender.sendMessage("§eHraci §7" + target.getName() + " §ebylo pridano §7" + value + " §ePrisCoinu");
                    } catch (NumberFormatException e) {
                        sender.sendMessage("§cMusis zadat cislo!");
                    }
                    break;
                case "take":
                    if (args.length > 3 || args.length < 3) {
                    ...
                    try {
                        ...
                        PrisCoins.takeCoins(target, value);
                        sender.sendMessage("§eHraci §7" + target.getName() + " §ebylo odebrano §7" + value + " §ePrisCoinu");
                    } catch (NumberFormatException e) {
                        sender.sendMessage("§cMusis zadat cislo!");
                    }
                    break;
                case "set":
                    ...
                    try {
                        if (!Main.getInstance().getMySQL().hasDataByName(args[1])) {
                            sender.sendMessage("§cTento hrac neexistuje");
                            break;
                        }
                        int value = Integer.valueOf(args[2]);
                        Player target = Bukkit.getPlayer(args[1]);
                        ...
                        PrisCoins.setCoins(target, value);
                        sender.sendMessage("§eHraci §7" + args[1] + " §ebyl stav uctu nastaven na §7" + value + " §ePrisCoinu");
                    } catch (NumberFormatException e) {
                        sender.sendMessage("§cMusis zadat cislo!");
                    }
                    break;
                default:
                    ...
                    sender.sendMessage("§eHrac " + args[0] + " §ema §7" + PrisCoins.getCoins(target) + " §ePrisCoinu!");
            }
        }
        return false;
    }
